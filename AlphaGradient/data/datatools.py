...
# This is a little bit hacky, but this this needs to be defined outside of
# the scope of AssetData even though it is only intended to be used in
# that class. This is because the COLUMNS enum defined within will not
# allow the use of subclasses as values for enum members. By defining it
# outside, we can use Value within the COLUMNS enum scope, allowing us to
# bypass the requirement that all values be the same. Ordinarily, we could
# just use 'settings=NoAlias', but it imposes too many restrictions when
# loading saved asset datasets from pickles.
