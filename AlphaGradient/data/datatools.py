...
def get_data(asset):
	...
	try:
		with open(path, "rb") as data:
			return AssetData(asset.__class__, pd.read_pickle(data))
	...
	def __init__(self, asset_type, data, columns=None):
		...
		if success:
			...
			if self._data.index[0].time() == time():
				self._data["_period_close_"] = [asset_type.market_close]
				self._data["_period_open_"] = [asset_type.market_open]
			else:
			...
			self._first = self._data["_period_open_"][0]
			self._last = self._data["_period_close_"][0]
