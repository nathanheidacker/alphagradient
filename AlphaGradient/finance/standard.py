...
class Currency(Asset):
	...
	def __init__(self, identifier='USD'):
		...
		if not isinstance(identifier, str):
			raise TypeError(f'{identifier.__class__.__name__} {identifier} is not a valid currency identifier. Please use a currency code or symbol as a string')
		...
		if len(identifier) == 1:
			...
			raise ValueError(f'{identifier} is not a valid currency identifier. Please use a currency symbol or three digit currency code')
...
class Stock(Asset):
	def __init__(self, ticker, date=None, data=None):
		optional = {'High':'float', 'Low':'float', 'Volume':'int', 'Adj Close':'float'}
		super().__init__(ticker, date=date, data=data, require_data=True, optional=optional)
	...
	def __init__(self, ticker=None, date=None):
		if ticker is None:
			...
			while ticker in TYPES.BROWNIANSTOCK.instances:
				ticker = ''.join([chr(np.random.randint(65, 91)) for _ in range(4)])
...
class Option(Asset, ABC):
	...
	def __init__(self, underlying, strike, expiry):
		...
		if not isinstance(expiry, datetime):
			raise TypeError(f'Invalid input {expiry=} for initialization of {underlying.name} {self.__class__.__name__}')
